...
[Sa,Sd,Sp]=DofSelection(d_cell,a_cell,p_cell,doflabel);

dt=0.02

[A B G J Ac Bc Gc Jc F]=ssmod_full(K,C,M,Sa,Sd,Sp,dt);


t=[0:dt:200];

p=[sin(t.*cos(t)/10) ; sin(t.*cos(t-10)/20) ];

Q_sim=eye(4)*1e-10;
R_sim=diag([0.01 0.01 0.1 0.1]);

w=mvnrnd(zeros(4,1),Q_sim,length(t)).';
v=mvnrnd(zeros(4,1),R_sim,length(t)).';

[x_sim,y_sim]=ssmod_forward_stoch(A,B,G,J,[],0,p,w,v);
...
[x_id,p_id]=JIS_trunc_ss(A,B,G,J,y_sim,x0,R_sim,Q_sim,S_sim,P01);

close all

plotTime(t,x_sim,x_id);
plotTime(t,p,p_id);
