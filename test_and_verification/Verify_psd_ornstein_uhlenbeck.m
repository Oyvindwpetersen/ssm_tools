...
S_eta_cont(1,1,1:length(omega_axis))=sigma_wc_squared/(2*pi);
H_ss_cont=ssmod_tf(Ac,Bc,G,J,omega_axis,[]);
S_y_ss_cont=mtimes3(H_ss_cont,S_eta_cont,H_ss_cont,'nnh');

H_ss_disc=ssmod_tf(A,B_sim,G,J,omega_axis,dt);
S_eta_disc(1,1,1:length(omega_axis))=sigma_wd_squared/(2*pi); % Not quite sure why this is, but it gives correct
...
S_theory(1,1,:)=(Bc*Gc)^2*sigma_wc_squared/(2*pi)./(lambda.^2+omega_axis.^2);
...
plotpsd(w_welch,S_welch,omega_axis,S_y_ss_cont*2,omega_axis,S_theory*2,plotopt);
