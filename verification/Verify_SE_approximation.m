...
[Fc,Lc,Hc,sigma_w,S_trunc]=ssmod_squaredexp(L,sigma_p,ns,w_axis); S_trunc_onesided=S_trunc*2;
...
% Exact PSD for SE
clear S_SE_exact
S_SE_exact(1,1,:)=exp(-0.5*L^2*w_axis.^2) * sigma_p^2*L/sqrt(2*pi);
S_SE_exact_onesided=S_SE_exact*2;
sigma_p_test=trapz(w_axis,S_SE_exact_onesided).^0.5

% PSD for state space model
Ht=ssmod_tf(Fc,Lc,Hc,zeros(1),w_axis,[],'type','io');
S_w=repmat(sigma_w.^2/(2*pi),1,1,length(w_axis));
S_ss_onesided=mtimes3(Ht,S_w,Ht,'nnh')*2;

plotSpectrum(w_axis,S_SE_exact_onesided,w_welch,S_welch,w_axis,S_ss_onesided,w_axis,S_trunc_onesided,...
    'xlabel','Frequency [rad/s]','ylabel','PSD',...
    'legend',{'SE onesided (exact)' 'Empirical (welch)' 'State-space onesided' 'Poly Taylor onesided'},...
    'LineStyle',{'-' '-' '--' ':'},...
    'log','no','xlim',[0 10]);
...
plotAutocorr(tau_axis,R_SE_exact,tau_emp,R_emp,...
    'xlabel','Time lag [s]','ylabel','ACF',...
    'legend',{'R SE (exact)'  'Empirical (xcorr)'},...
    'LineStyle',{'-' '-' '--' ':'},...
    'log','no','xlim',[0 50]);
